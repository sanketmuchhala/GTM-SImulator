            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generation complete, but entire section may not be done yet) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM updates, adjust scroll position to keep button in same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics, supporting current English and legacy Chinese formats
    const factMatch = text.match(/Related prediction facts:\s*(\d+)/) || text.match(/\u76f8\u5173\u9884\u6d4b\u4e8b\u5b9e:\s*(\d+)/)
    const entityMatch = text.match(/Involved entities:\s*(\d+)/) || text.match(/\u6d89\u53ca\u5b9e\u4f53:\s*(\d+)/)
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/\*\*Interviewee count:\*\*\s*(\d+)\s*\/\s*(\d+)/) || text.match(/\*\*\u91c7\u8bbf\u4eba\u6570:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*select\s+([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/i) || line.match(/^-\s*\u9009\u62e9([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^Not selected|^In summary|^Final selection/i) && !line.match(/^\u672a\u9009|^\u7efc\u4e0a|^\u6700\u7ec8\u9009\u62e9/)) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key quotes|$)/) || block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*\u5173\u952e\u5f15\u8a00|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()

        // Separate Twitter and Reddit answers
        const twitterMatch = answerText.match(/\[Twitter answer\]\n?([\s\S]*?)(?=\[Reddit answer\]|$)/) || answerText.match(/【Twitter\u5e73\u53f0\u56de\u7b54】\n?([\s\S]*?)(?=【Reddit\u5e73\u53f0\u56de\u7b54】|$)/)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const countMatch = text.match(/Found\s*(\d+)\s*items?/) || text.match(/\u627e\u5230\s*(\d+)\s*\u6761/)
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. Legacy Chinese "Question X:" format
      // 2. "1. " or "\n1. " (digit + dot, legacy format)
      let matches = []
      let match

      // Prefer the legacy Chinese question marker when present
      const cnPattern = /(?:^|[\r\n]+)\u95ee\u9898(\d+)[\uff1a:]\s*/g
      ...
      // Fall back to digit. format if no matches
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform toggle buttons (shown only when there are real dual-platform answers)
...
const renderMarkdown = (content) => {
  ...
  // Clean <br> immediately after block closing tags before paragraph opening tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol> is separated by paragraph content, keep numbering incremental
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after the legacy Chinese "Final Answer:" marker
  ...
  // If content is long and contains markdown format, try removing thought process then return
