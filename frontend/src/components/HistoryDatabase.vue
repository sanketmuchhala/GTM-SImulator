...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate number of cards in current row to ensure each row is centered
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Shorter delay (50ms) when expanding, longer delay (200ms) when collapsing for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay slightly before executing to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
.grid-pattern {
  ...
  /* Position from top-left corner; height changes only expand at bottom, not affecting existing grid position */
...
.cards-container {
  ...
  /* min-height dynamically computed by JS, adapts based on card count */
}
