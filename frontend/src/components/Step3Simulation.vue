...
// Computed
// Display actions chronologically, with the newest at the bottom
...
// Format elapsed simulation time based on rounds and minutes per round
...
const doStartSimulation = async () => {
  ...
  // Reset all state first so the previous simulation does not affect this run
  resetAllState()
...
// Track each platform's previous round to detect changes and write logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect each platform's round changes separately and write logs
      ...
      // Extra check: the platform may report completion before the backend updates runner_status
      // Determine this through twitter_completed and reddit_completed
...
const checkPlatformsCompleted = (data) => {
  ...
  // Check each platform's completion state
  const twitterCompleted = data.twitter_completed === true
  const redditCompleted = data.reddit_completed === true
  
  // If at least one platform has completed, check whether all enabled platforms are done
  // Determine whether a platform was enabled through actions_count or a previous running state
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Add new actions incrementally with deduplication
