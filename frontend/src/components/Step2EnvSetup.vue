...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_agent_profiles' || newStage === 'generating_profiles' || newStage === '\u751f\u6210Agent\u4eba\u8bbe') {
    phase.value = 1
  } else if (newStage === 'generating_simulation_config' || newStage === 'generating_config' || newStage === '\u751f\u6210\u6a21\u62df\u914d\u7f6e') {
    phase.value = 2
  ...
  } else if (newStage === 'preparing_simulation_scripts' || newStage === 'copying_scripts' || newStage === '\u51c6\u5907\u6a21\u62df\u811a\u672c') {
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds is at least 40 (recommended value) to avoid slider range anomalies
...
// Calculate total related topic count for all personas
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check whether preparation has completed
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Update only when the API returns valid values to avoid overwriting existing valid data
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output config generation stage logs without duplication
