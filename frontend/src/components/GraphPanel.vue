...
const renderGraph = () => {
  ...
  // Process edge data, calculate count and index of edges between the same node pair
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // This ensures all edges are distributed in the same reference frame, not overlapping due to different directions
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravity toward center so isolated node groups cluster toward center
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc departing from node and returning
      const loopRadius = 30
    ...
    // Perpendicular offset to the line direction, calculated by distance ratio to ensure visible curve
    // More edges = larger offset proportion relative to distance
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if real dragging has started (moved beyond threshold)
