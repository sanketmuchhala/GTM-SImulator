...
const renderMarkdown = (content) => {
  ...
  // Clean <br> before a paragraph that immediately follows a block-level element
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists split by paragraph content
...
const sendMessage = async () => {
  ...
  try {
    ...
    scrollToBottom()
    // Automatically save conversation history to the cache
    saveChatHistory()
  }
}
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    // Convert the object dictionary to an array and prefer the Reddit platform response
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // Prefer the Reddit platform response, then Twitter
...
/* Left panel - same report style as Step4Report.vue */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* Fix ordered list numbering with CSS counters so multiple ol elements continue numbering */
.message-text {
