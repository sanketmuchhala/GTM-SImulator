...
- After the simulation completes, the environment is not immediately closed; enters command-waiting mode
...
# This fixes the issue where the OASIS third-party library reads files without specifying encoding
# ============================================================
import sys
import os
...
# Load .env file from the project root directory (contains LLM_API_KEY and other configs)
...
# Add the filter immediately when the module loads, before any camel code executes
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (recording every agent's observations and actions);
...
class ParallelIPCHandler:
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Needs interviews on both platforms
...
# Non-core action types to be filtered out (these actions have low analytical value)
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Maximum rowid value read last time (using rowid instead of created_at
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keeping only key fields (preserve full content without truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    try:
        ...
        # Post comment: enrich with the post being commented on
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If a boost LLM is configured, parallel simulation can have different platforms
    use different API providers to improve concurrency.
    ...
    # Choose which LLM to use based on arguments and available configuration
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum number of simulation rounds (optional; truncates excessively long simulations)
    ...
    # Use OASIS default name if an agent is missing from config
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed database rowid to avoid created_at format differences
    
    # Execute initial events
    event_config = config.get("event_config", {})
    initial_posts = event_config.get("initial_posts", [])
    
    # Record round 0 start for initial events
    if action_logger:
    ...
    for round_num in range(total_rounds):
        ...
        # Record round start even when there are no active agents
        if action_logger:
            action_logger.log_round_start(round_num + 1, simulated_hour)
        
        if not active_agents:
            # Record round end with actions_count=0 when there are no active agents
            if action_logger:
        ...
        # Get actual executed actions from database and record them
    ...
    # Do not close the environment; keep it for Interview use
...
async def run_reddit_simulation(
    ...
    # Reddit Use accelerated LLM config if available, otherwise fall back to common config
...
async def main():
    ...
    # Initialize logging, disable OASIS logs, and clean old files
    ...
    if args.twitter_only:
        ...
        # Run in parallel with separate loggers per platform
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop completed! Total elapsed time: {total_elapsed:.1f}seconds")
...
def setup_signal_handlers(loop=None):
    """
    Set signal handlers so SIGTERM/SIGINT exits cleanly
    
    Persistent simulation: stay alive after simulation and wait for interview commands
    When a termination signal is received:
    1. Notify asyncio loop to stop waiting
    2. Let the program clean up resources such as databases and environments
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit and clean resources
            if _shutdown_event:
                _shutdown_event.set()
        
        # Do not call sys.exit() directly; let asyncio exit and clean resources
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean multiprocessing resource tracker to prevent exit warnings
        try:
