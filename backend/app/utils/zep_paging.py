"""Zep Graph paginated retrieval utilities.

The Zep node/edge list API uses UUID cursor-based pagination.
This module encapsulates the auto-pagination logic (with per-page retry) and
...
def fetch_all_nodes(
    ...
    """Fetch graph nodes with pagination, returning at most max_items entries (default 2000). Each page request includes its own retry logic."""
    all_nodes: list[Any] = []
