...
INTERVIEW_PROMPT_PREFIX = "Drawing on your persona, all past memories and actions, reply to me directly in plain text without calling any tools: "
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match predefined entity types (nodes whose Labels are not exclusively Entity).
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    # List of required files (scripts are not included; they reside in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - ready: preparation complete, simulation can run
        # - preparing: if config_generated=True, preparation is also done
        # - running: simulation is running, so preparation finished long ago
        # - completed: run finished, so preparation finished long ago
        ...
        if status in prepared_statuses and config_generated:
            ...
            # If the status is "preparing" but all files are ready, auto-update the status to "ready"
            if status == "preparing":
                try:
...
def prepare_simulation():
    ...
    - Returns existing results immediately if preparation is already complete
    ...
    4. Use the LLM to intelligently generate the simulation configuration (with retry logic)
    ...
    try:
        ...
        # Fetch the simulation requirement
        ...
        # This allows the frontend to immediately obtain the expected total number of Agents after calling /prepare.
        try:
    ...
    Request (JSON):
        {
            ...
            "simulation_id": "sim_xxxx"      // optional, simulation ID (for checking completed preparations)
        }
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Iterates the reports directory to find reports whose simulation_id matches;
...
def get_simulation_history():
    ...
    Used for the home page history display; returns a rich simulation list including project name, description, etc.
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Fetch simulation config info (reads simulation_requirement from simulation_config.json)
            ...
            # Fetch run state (reads the user-configured actual round count from run_state.json)
            ...
            # Fetch associated project files (up to 3)
            ...
            # Fetch the associated report_id (find the most recent report for this simulation)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulation Agent Profiles in real time (for live progress monitoring during generation).
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get the simulation configuration in real time (for live progress monitoring during generation).

    Differences from /config:
    - Reads the file directly, bypassing SimulationManager
    - Suitable for real-time viewing during the generation process
    - Returns extra metadata (e.g. file modification time, whether generation is in progress)
    - Can return partial information even if the configuration has not been fully generated yet
...
def get_simulation_config(simulation_id: str):
    """
    Get the simulation configuration (the complete configuration generated intelligently by the LLM).
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "enable_graph_memory_update": false,   // optional: whether to dynamically update Agent activities to the Zep graph memory
            "force": false                         // optional: force restart (stops the running simulation and clears logs)
        }

    About the force parameter:
        - When enabled, if the simulation is running or completed, it will be stopped first and run logs will be cleared.
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities in the simulation (posting, commenting, liking, etc.) are updated to the Zep graph in real time.
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                # Preparation is complete; check whether a process is already running
        ...
        # Fetch graph ID (for graph memory update)
        graph_id = None
...
def get_run_status(simulation_id: str):
    """
    Get the simulation's real-time run status (for frontend polling).
...
def get_run_status_detail(simulation_id: str):
    ...
    Used for real-time display on the frontend.
    ...
    try:
        ...
        # Fetch actions for the current round (recent_actions shows only the latest round)
...
def get_simulation_actions(simulation_id: str):
    """
    Get the Agent action history for the simulation.
...
def get_agent_stats(simulation_id: str):
    ...
    Used to display Agent activity rankings and action distribution on the frontend.
    """
    try:
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // If not specified: interviews both platforms simultaneously in dual-platform mode
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // optional, default platform (overridden per item)
                                               // If not specified: each Agent is interviewed on both platforms simultaneously
    ...
    try:
        ...
        # Optimize each interview prompt by adding a prefix to prevent Agent from calling tools
