...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        # Return directly in "agent_name: activity description" format, without simulation prefix
...
class ZepGraphMemoryUpdater:
    ...
    Activities are grouped by platform and sent to Zep in batches after
    accumulating BATCH_SIZE entries.

    All meaningful actions are updated in Zep; action_args will include full context:
    ...
    # Batch send size (how many entries per platform to accumulate before sending)
    BATCH_SIZE = 5
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will include full context information (e.g. post text, usernames).
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send a batch of activities to the Zep graph (merged into a single text entry).
    ...
    def _flush_remaining(self):
        ...
        # Then send remaining activities from each platform's buffer (even if fewer than BATCH_SIZE)
        with self._buffer_lock:
