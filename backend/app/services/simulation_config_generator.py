...
Enables full automation with no manual parameter configuration required.

Adopts a step-by-step generation strategy to avoid failures from generating
excessively long content all at once:
...
class SimulationConfigGenerator:
    ...
    Analyzes simulation requirements, document content, and graph entity
    information using an LLM to automatically generate optimal simulation
    ...
    def _try_fix_config_json(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        if json_match:
            ...
            # Remove newlines from within strings
...
- Infer the target user group's timezone and daily schedule from the simulation scenario; the following is a reference example for UTC+8 (China Standard Time)
...
- 19–22 PM: peak hours (activity multiplier 1.5)
- After 23: activity declining (activity multiplier 0.5)
- General pattern: low activity in the early morning, gradually increasing in the morning, moderate during work hours, peak in the evening
- **Important**: The example values below are for reference only; you need to adjust the specific time slots based on the nature of the event and characteristics of the participant group
  - Example: student groups may peak at 21–23; media is active all day; official institutions are only active during work hours
  - Example: a breaking news event may spark discussion late at night, so off_peak_hours can be shortened accordingly
...
- peak_hours (int array): Peak hours; adjust based on the event's participant group
...
- reasoning (string): Brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON format; the time configuration should match the daily schedule of the target user group in the simulation scenario."
...
**Important**: poster_type must be chosen from the "Available entity types" above, so that initial posts can be assigned to the appropriate Agent for posting.
For example: official announcements should be posted by Official/University type, news by MediaOutlet, student opinions by Student.
...
- **Times should match the target user group's daily schedule**: the following are reference values (UTC+8); adjust based on the simulation scenario
...
{{
    "agent_configs": [
        {{
            ...
            "active_hours": [<list of active hours, considering the target group's schedule>],
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON; the configuration should match the daily schedule of the target user group in the simulation scenario."
        ...
        for i, entity in enumerate(entities):
            ...
            # If the LLM did not generate a config, use rule-based generation
            if not cfg:
