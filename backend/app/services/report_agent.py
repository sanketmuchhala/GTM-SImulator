...
2. Plans the table of contents first, then generates content section by section
3. Each section uses a ReACT multi-round thinking and reflection loop
4. Supports conversational interaction, with autonomous tool calls during dialogue
"""
...
class ReportLogger:
    ...
    def log_section_full_complete(
        ...
        finished and to retrieve its full content.
        """
        self.log(
...
class ReportConsoleLogger:
    ...
    inside the report folder. These differ from agent_log.jsonl in that they are plain-text
    console output rather than structured JSON.
    """
    ...
    def _setup_file_handler(self):
        """Set up a file handler to mirror log output to the file"""
        import logging
...
3. Help you understand how public sentiment has evolved

[When to use]
- When you need the full development arc of an event
- When you need to compare sentiment changes across different phases
...
[What it returns]
- A list of facts most relevant to your query"""
...
running simulation Agents! This is not an LLM simulation — it calls the actual interview
interface to get raw responses from simulation Agents. By default, interviews are conducted
simultaneously on both Twitter and Reddit to obtain more comprehensive perspectives.
...
[When to use]
- When you need to understand an event from different role perspectives (How do students see it? Media? Officials?)
- When you need to gather opinions and positions from multiple parties
- When you need real responses from simulation Agents (from the OASIS environment)
...
[Core Concept]
We have built a simulation world and injected a specific "simulation requirement" as a variable. The evolution of the simulation world represents a prediction of what could happen in the future. What you are observing is not "experimental data" but a "rehearsal of the future."
...
You are an expert author of "Future Prediction Reports" and are currently writing one section of the report.
...
(simulation requirements) into the simulation world. The behavior and interactions of
...
1. [You MUST call tools to observe the simulation world]
   ...
   - All content must come from events and Agent words/actions within the simulation world
   - It is forbidden to use your own knowledge to write report content
   - Call tools at least 3 times (maximum 5 times) per section to observe the simulated world,
...
3. [Language consistency - quoted content must be translated into the report language]
   - Content returned by tools may contain language different from the report language
   - The entire report must be written in the language specified by the user
   - When quoting tool-returned content in another language, you must translate it into
     the report language before including it
   - Preserve the original meaning and ensure natural, fluent expression
...
═══════════════════════════════════════════════════════════════
[⚠️ Formatting Rules - Critically Important!]
═══════════════════════════════════════════════════════════════
...
- ❌ Do NOT add a section heading at the start of the content
...
[Correct example]
```
This section analyzes the public opinion spread of the event. Through in-depth analysis
of the simulation data, we found...

**Initial Ignition Phase**

Platform X served as the primary venue for information breaking, driving the core
initial spread:

> "Platform X accounted for 68% of initial content volume..."
...
The system will execute the tool and return the result to you. You must not write the tool
result yourself.

Option B - Output the final content:
Once you have gathered enough information through tools, output the section content
beginning with "Final Answer:"
...
4. [Blockquote format rules - must stand as its own paragraph]
   Blockquotes must be standalone paragraphs with a blank line before and after;
   they must not be embedded within a paragraph:

   ✅ Correct format:
   ```
   The institution's response was seen as lacking substance.

   > "The institution's approach appeared rigid and slow in the fast-moving social media environment."
   ...
   ❌ Incorrect format:
   ```
   The institution's response was seen as lacking substance. > "The institution's approach..." This assessment reflects...
   ```
...
3. Once you have gathered enough information, output the Final Answer (body text only — no headings)"""
...
- If you need more information: call one tool to continue retrieval
═══════════════════════════════════════════════════════════════"""
...
REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "You have only called tools {tool_calls_count} time(s) so far, but at least {min_tool_calls} are required. "
...
[Answer Style]
- Be concise and direct — no lengthy essays
...
class ReportAgent:
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only tried when format 1 did not match, to avoid false positives on JSON in body text
    ...
    def _generate_section_react(
        ...
        4. Repeat until enough information is gathered or the maximum iterations are reached
        5. Final Answer - generate the section content

        Args:
            section: The section to generate
            outline: The complete outline
            previous_sections: Content from previous sections (used to maintain coherence)
        ...
        # Build user prompt - each completed section capped at 4000 characters
        if previous_sections:
            previous_parts = []
            for sec in previous_sections:
                # Each section capped at 4000 characters
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returned None; break out and fall into forced wrap-up
                break
            ...
            # ── Conflict handling: LLM output both a tool call and a Final Answer in the same reply ──
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    # First two times: discard this response, ask the LLM to reply again
                    messages.append({"role": "assistant", "content": response})
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] Your reply contained both a tool call and a Final Answer simultaneously, which is not allowed.\n"
            ...
            if has_tool_calls:
                ...
                # Build the hint for unused tools
                unused_tools = all_tools - used_tools
                unused_hint = ""
            ...
            # Use the content directly as the final answer without wasting another iteration.
        ...
        # Check whether the LLM returned None during forced wrap-up
        if response is None:
    ...
    def generate_report(
        ...
        """
        Generate a complete report (streaming output section by section)
        ...
        # If no report_id was provided, generate one automatically
        if not report_id:
        ...
        try:
            ...
            # Phase 2: Generate section by section (save each section as it completes)
            report.status = ReportStatus.GENERATING

            total_sections = len(outline.sections)
            generated_sections = []  # Stores generated content for use as context
    ...
    def chat(
        ...
        The agent can call retrieval tools during chat to answer questions
        ...
        for iteration in range(max_iterations):
            ...
            tool_results = []
            for call in tool_calls[:1]:  # Execute at most one tool call per iteration
...
class ReportManager:
    ...
    File structure with sectioned output:
    reports/
      {report_id}/
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        This is console output from report generation, such as INFO and WARNING,
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                total_lines = i + 1
                if i >= from_line:
                    # Keep original log line and strip trailing newline
    ...
    def get_console_log_stream(cls, report_id: str) -> List[str]:
        """
        Get complete console log in one read
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get complete Agent log in one read
    ...
    def save_outline(cls, report_id: str, outline: ReportOutline) -> None:
        ...
        Called immediately after planning completes
        """
    ...
    def save_section(
        ...
        Called after each section is generated to support sectioned output
        ...
        # Build section Markdown content and clean duplicate headings
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        2. Convert all headings at ### and below into bold text
        ...
        for i, line in enumerate(lines):
            ...
            # Check whether it is a Markdown heading line
            ...
            if heading_match:
                ...
                # Check whether heading duplicates the section title in the first 5 lines
                if i < 5:
                ...
                # Section titles are added by the system, so content should not contain headings
                cleaned_lines.append(f"**{title_text}**")
                cleaned_lines.append("")  # Add blank line
                continue
            
            # If previous line was skipped heading and current line is empty, skip it too
    ...
    def update_progress(
        ...
        Frontend can read progress.json for realtime progress
        """
        cls._ensure_report_folder(report_id)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble full report from saved section files and clean headings
        """
        folder = cls._get_report_folder(report_id)
        ...
        # Post-process heading issues in the full report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep the report title (#) and section titles (##), and convert other heading levels
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check duplicate headings within 5 nearby lines
                is_duplicate = False
                ...
                if level == 1:
                    ...
                    # ### and lower headings are converted to bold text
                    processed_lines.append(f"**{title}**")
                    processed_lines.append("")
                    prev_was_heading = False
        ...
        # Clean consecutive blank lines, keeping at most two
        result_lines = []
        empty_count = 0
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Legacy compatibility: check files stored directly under reports
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        # Legacy format: delete separate files
        deleted = False
